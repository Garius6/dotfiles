...
windows_string = '\n'.join([f"<{app_id}> {app_name}" for app_id, app_name, app_title in windows])

active_display = subprocess.check_output('~/.config/sway/scripts/get_active_display.sh', shell=True).decode().strip()
